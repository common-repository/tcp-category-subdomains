...
if (!function_exists('is_plugin_installed_and_activated')) {
	/**
	 ...
	 * @param string $plugin Name of plugin that depend on this plugin, e.g TCP Products Sync
	 * @return bool
	 */
	function is_plugin_installed_and_activated($id, $name, $file, $plugin) {
		...
		add_action('admin_notices', function() use ($id, $name, $file, $plugin) {
			...
			if ($page == 'update.php' && $action == 'install-plugin') {
				return;
			}
			$installed = file_exists(WP_PLUGIN_DIR .'/'. $file);
			$msg = 'Please ';
			if ($installed) {
				$msg .= '<a href="'. wp_nonce_url(admin_url('plugins.php?action=activate&plugin='. urlencode($file) .'&plugin_status=all&paged=1&s'), 'activate-plugin_' . $file) .'">activate</a>';
			} else {
				$msg .= '<a href="'. wp_nonce_url(admin_url('update.php?action=install-plugin&plugin='. $id), 'install-plugin_'. $id) .'">install</a> and activate';
			}
			$msg .= ' '. $name .' plugin in order to use with <b>'. $plugin .'</b>.';
...
if (!function_exists('enable_error_log')) {
	/**
	 * Enable logging PHP errors into custom logfile and view from URL https://<domain>/?tcp_error_log=1
	 * To delete this custom logfile, https://<domain>/?tcp_error_log_reset=1
	 * Note: doesn't work in kitoks, maybe write permission issue
	 */
	function enable_error_log() {
		...
		$err_log_file = dirname(ABSPATH) .'/tcp-php-errors.log';
		@ini_set('log_errors', 'On');
		@ini_set('error_log', $err_log_file);
		$fs = get_wp_filesystem(dirname($err_log_file));
		if (!file_exists($err_log_file)) {
			$fs->put_contents($err_log_file, '');
		}
		add_route('tcp_error_log', function() use ($fs, $err_log_file) {
			$logs = $fs->get_contents($err_log_file);
			echo '<pre>'. htmlentities($logs) .'</pre>';
		});
		add_route('tcp_error_log_reset', function() use ($fs, $err_log_file) {
			$fs->delete($err_log_file);
		});
		$enabled = true;
	}
}
...
if (!function_exists('add_route')) {
	/**
	 * @param string $route query var name
	 * @param callable $callback
	 */
	function add_route($route, $callback) {
		static $attached = false;
		// static $attached_activation_hook = [];
		if (!isset($GLOBALS['_qvcb'])) {
			$GLOBALS['_qvcb'] = [];
		}
		$GLOBALS['_qvcb'][$route] = $callback;
		/*$add_rewrites = function() {
			if (isset($GLOBALS['_qvcb'])) {
				foreach (array_keys($GLOBALS['_qvcb']) as $route) {
					add_rewrite_rule($route . '/?$', 'index.php?'. $route .'=1', 'top');
					add_rewrite_rule($route . '/(.+)/?$', 'index.php?'. $route .'=$matches[1]', 'top');
				}
			}
		};
		...
		$tmp_parts = [];
		foreach ($file_parts as $i => $p) {
			$tmp_parts[] = $p;
			if ($p == 'plugins' && $file_parts[$i - 1] == 'wp-content') {
				$plugin_folder = $file_parts[$i + 1];
				$tmp_parts[] = $plugin_folder;
				$plugin = get_plugins('/' . $plugin_folder);
				if (!empty($plugin)) {
					$main_file = implode('/', $tmp_parts) .'/'. key($plugin);
					if (!isset($attached_activation_hook[$main_file])) {
						register_activation_hook($main_file, function() use ($add_rewrites) {
							$add_rewrites();
							flush_rewrite_rules();
						});
						$attached_activation_hook[$main_file] = 1;
					}
				}
				break;
			}
		...
		add_filter('query_vars', function($vars) {
			if (isset($GLOBALS['_qvcb'])) {
				$vars = array_merge($vars, array_keys($GLOBALS['_qvcb']));
			}
			return $vars;
		});
		add_action('parse_request', function($wp) {
			if (isset($GLOBALS['_qvcb'])) {
				foreach ($GLOBALS['_qvcb'] as $var => $callback) {
					if (array_key_exists($var, $wp->query_vars)) {
						$args = [$wp->query_vars[$var]];
...
if (!function_exists('get_wp_filesystem')) {
	/**
	 * @param string $folder folder to test for connection to determine type of filesystem class to return (direct, ftp, etc.)
	 * @return WP_Filesystem_Direct
	 */
	function get_wp_filesystem($folder = null) {
		static $fs = null;
		if ($fs) {
			return $fs;
		}
		...
		WP_Filesystem(false, $folder);
		$fs = $GLOBALS['wp_filesystem'];
		if (get_class($fs) != 'WP_Filesystem_Direct' && !function_exists('wp_generate_password')) {
